...
const memories: Record<string, { title: string; date: string; image: string; music?: string }> = {
  "first-picture": {
    title: "Our First Picture Together",
    date: "January 17, 2025",
    image: "/memories/first-picture.jpg",
    music: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/file_example_MP3_700KB-Q6wkRF4iULyR7xhFHDHNw5R5055mDM.mp3",
  },
  "the-day-you-smiled": {
    ...
    return (
      <main className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="font-serif text-2xl text-foreground mb-4">Memory not found</h1>
    ...
    <main className="min-h-screen bg-gradient-to-b from-background via-background to-secondary/20 overflow-hidden relative">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        {heartPetals.map((petal, i) => (
          <RosePetal key={`heart-${i}`} delay={petal.delay} startX={petal.startX} />
        ))}
        {rosePetals.map((petal, i) => (
          <FloatingPetal key={`petal-${i}`} delay={petal.delay} startX={petal.startX} />
        ))}
      </div>
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-6 py-20">
        ...
        >
          <p className="text-primary text-sm uppercase tracking-[0.3em] mb-2">{memory.date}</p>
          <h1 className="font-serif text-3xl md:text-5xl text-foreground">{memory.title}</h1>
        </motion.div>
